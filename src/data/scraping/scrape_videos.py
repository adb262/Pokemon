...
class PokemonVideoScraper:
        ...
        if use_s3:
            if s3_manager is None:
                try:
                    ...
                    logger.info(
                        f"S3 manager initialized for bucket: {self.s3_manager.bucket_name}"
                    )
    ...
    def _load_downloaded_list(self) -> set:
        ...
        if self.use_s3 and self.s3_manager:
            ...
            if downloaded_data:
                logger.info(
                    f"Loaded downloaded videos list from S3: {len(downloaded_data)} videos"
                )
        ...
        if downloaded_file_local.exists():
            with open(downloaded_file_local, "r") as f:
                data = json.load(f)
                logger.info(
                    f"Loaded downloaded videos list from local file: {len(data)} videos"
                )
                return set(data)
    ...
    def _filter_video_info(self, info: Dict) -> bool:
        ...
        if duration < 600 or duration > 28800:
            logger.info(
                f"Skipping video {info.get('id')} - duration {duration}s outside range"
            )
            return False
        ...
        if any(keyword in title for keyword in bad_keywords):
            logger.info(
                f"Skipping video {info.get('id')} - contains bad keyword in title"
            )
            return False
        ...
        if not any(keyword in title for keyword in good_keywords):
            logger.info(
                f"Skipping video {info.get('id')} - no gameplay keywords in title"
            )
            return False
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Pokemon Video Scraper with S3 support"
    )
    parser.add_argument(
        "--output-dir", default="raw_videos", help="Local output directory"
    )
    parser.add_argument(
        "--max-videos", type=int, default=30, help="Max videos per game"
    )
    parser.add_argument("--use-s3", action="store_true", help="Upload videos to S3")
    parser.add_argument(
        "--s3-prefix", default="raw_videos", help="S3 prefix for videos"
    )
    parser.add_argument(
