...
def create_tensorboard_writer(
    config: TrainingConfig, s3_manager: Optional[S3Manager] = None
):
        ...
        for root, dirs, files in os.walk(config._temp_tensorboard_dir):
            for file in files:
                ...
                relative_path = os.path.relpath(
                    local_path, config._temp_tensorboard_dir
                )
                ...
                if success:
                    ...
                    logging.warning(
                        f"Failed to upload tensorboard file to S3: {s3_key}"
                    )
