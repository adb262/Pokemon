...
from idm.codebook import NaiveCodebook
from idm.lapa.positional_bias import Transformer
...
class VQVAE(nn.Module):
    ...
    def quantize(self, encoded_first: torch.Tensor, encoded_last: torch.Tensor) -> torch.Tensor:
        encoded_first = encoded_first.reshape(encoded_first.shape[0], -1)
        encoded_last = encoded_last.reshape(encoded_last.shape[0], -1)
        quantized_representation: torch.Tensor = self.codebook(encoded_first, encoded_last)
